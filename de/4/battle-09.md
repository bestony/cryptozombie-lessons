...
material:
  editor:
    ...
    answer: >
      ...
      external ownerOf(_zombieId) { Zombie storage myZombie =
      zombies[_zombieId]; Zombie storage enemyZombie = zombies[_targetId]; uint
      rand = randMod(100); if (rand <= attackVictoryProbability) {
      myZombie.winCount++; myZombie.level++; enemyZombie.lossCount++;
      feedAndMultiply(_zombieId, enemyZombie.dna, "zombie"); } else {
      myZombie.lossCount++; enemyZombie.winCount++; _triggerCooldown(myZombie);
      } } }
---
...
1. Add an `else` statement. If our zombie loses:
   ...
   `feedAndMultiply`. So the zombie's cooldown will be triggered whether he wins
   or loses.)
