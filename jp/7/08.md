...
前のチャプターの続きをやっていこう。今度はプレイヤーのデッキにある各カードを反復
して、禁止されていないかチェックしたい。

## プレイヤーのデッキにあるカードをループ

ここでは両プレイヤーのデッキにある各カードを反復して、全てを"Slab"に変更していく
例をやっていくぞ。ちなみに Slab はファンに人気のカードだ 😉
...
見ての通り、最初の各プレイヤーリストの反復にはネスト化された`for`ループを使って
いて、そのあとで各プレイヤーのカードリストを反復している。

> 注: プレイヤーのデッキにあるカードの`mouldName`を変更することで、そのカードは
> 完全に Slab という名前のカードに交換されます。これは単なる表示名ではありません
> 。

## さあテスト�

1. ２つ目の`for`ループを作り、上記のように各プレイヤーの`cardsInDeck`全てを反復
   するようにせよ。

2. 関数`isLegalCard`を呼び出し、これ
   に`gameState.playerStates[i].cardsInDeck[j]`を渡す`if`ステートメントを作成せ
   よ。この関数は次のチャプターで定義していくからな。

3. `if`ステートメントの中
   で`newCards[cardCount]`が`gameState.playerStates[i].cardsInDeck[j]`と等しくな
   るように設定することで、カードを新たな配列にコピーせよ。
