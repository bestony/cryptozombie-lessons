...
ネスト化された`for`ループの実行が終わったら、両プレイヤーのカードを禁止されたカ
ードを含まない新たなセットで置き換えるんだったな(ただしプレイヤーのデッキに最初
からカードがなかった場合は、何も変更されない)。

あとは各プレイヤーのスタート守備値や Goo ボトルの変更も行った。

しかし`changes`は単にメモリで保存されるローカルデータタイプであったことを思い出
そう。我々のゲームモードを完成させるには、このデータをコアのゲームロジックへと返
さなくてはならないのだ。
...
このイベントは、我々が行いたい全変更のデータを`serializedGameState`と同様の形式
で含んでいる。
