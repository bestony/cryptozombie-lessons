...
チャプター３で初期化した`GameState`を使って、現在のゲーム状態からデータを読み取
れるようになったな。

`beforeMatchStart()`の最後では、そのデータを`SerializedGameState`としてゲームサ
ーバーに返す必要があるのだ。

このためには、我々が行った変更全てをトラックしてくれ
る`ZBSerializer.SerializedGameStateChanges`という名のカスタムデータタイプを宣言
しなくてはならない。
...
`changes`ではその名のとおり、この関数の最後で返却されるゲーム状態に全ての変更を
行う。
...
これでデータ保存を始める準備が整うのだ。`gameState`とは違って
、`changes.init()`は引数を必要としない。

## さあテスト�

1. `changes`という`memory`で、`ZBSerializer.SerializedGameStateChanges`を宣言せ
   よ。
