...
ゲームモード内で禁止されているカードのリストが定義できたから、今度は各プレイヤー
のデッキのカードをチェックして、許可されていないものを取り除くためのロジックを書
かなくてはならない。

Solidity の配列にある要素を消去するのは単純なことではないから、メモリで新たな配
列を作成して全ての「許可された」字カードをコピーする方が簡単だ。

ではこれを行うために必要なものをやっていくぞ。

## プレイヤーリストの反復

`gameState`構造体は、両プレイヤーとそのカードも含めた現在のゲーム状態の全詳細を
持っていることを思い出すのだ。
...
`CardInstance` は ZBGameMode の構造体で、カードのデータを保存している:
...
許可されたカードを１つずつコピーすることで、各プレイヤーの新たなカード配列を作る
ことができる。以下のようにできるぞ:
...
2. 各プレイヤーに、`newCards`という`CardInstance`の配列を作成せよ。配列の長さは
   `gameState.playerStates[i].cardsInDeck.length`でなくてはならない。
