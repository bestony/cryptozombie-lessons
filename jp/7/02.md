...
Zombie Battleground カスタムゲームモードのコントラクトのシェルは、右にあるように
Solidity で書かれた他のスマートコントラクトと同じように始まる。

まず注意すべきは、我々は`ZBGameMode.sol`という名前のファイルからインポートしてお
り、このファイルも`ZBSerializer.sol`とその他いくつかのファイルをインポートしてい
るということだ。

> 注: これらその他のファイルの中には、非常にサイズの大きいものがあります。そのた
> めこのレッスンではそれらを含めることはしていません。ソース完全版に
> は<a href="https://github.com/loomnetwork/zb_game_mode" target=_blank>こちらの
> github リポジトリ</a>よりアクセスしていただけます。しかしこれらファイルのコン
> テンツを読んで理解しようとご心配いただく必要はありません。カスタムゲームモード
> 作成に必要な関連知識をこれから説明していきます。

我々のコントラクト`ExampleGame`は`ZBGameMode`を継承している。だからたくさんの共
通のデータ構造とヘルパーメソッドにアクセスすることができるのだ。カスタムゲーム開
発者としてのお主の日々をイージーにするために、これらを作成しておいたからな。

それと我々はこれをライブラリに抽象化しておいたから、各カスタムゲームモードに同じ
定型ロジックを書く必要はない。

### "フック" — カスタムゲームモードの核心

全てのカスタムゲームモードにとって最初のステップとなるのは、少なくとも１つの
**_フック_** を実装することだ。

コアのゲームロジックは基本的に、コードにあればゲームのある部分においてある関数を
コントラクトから探し呼び出す。

例えば、`beforeMatchStart`は対戦がスタートする前にのみ呼び出され
、`afterInitialDraw`はプレイヤーが最初のカードを引いた後すぐ呼び出される
。`beforeTurnStart` _(計画しているが未実装)_ は各プレイヤーのターンスタート時に
発動する。

これらのフックは、ゲームが続きを始める前にお主のカスタムルールを適用し、その時点
でのゲーム状態を「利用」して編集することを可能にするのだ。

我々が実装する最初のフックは`beforeMatchStart`というものだ。これはその名前の示す
とおり、各対戦のスタート前に適用されるフックだ。
...
この関数内で、対戦スタート時に適用したいゲームロジックへのカスタム変更の全てを定
義するのだ。

これらの変更というのは、個別のカードのバランス変更とか、どのカードがこのゲームモ
ードで許可/禁止されるかというルール、さらにオーバーロードの体力や Goo ボトルのス
タート数といったようなものとなる。

> Zombie Battleground をプレイしたことがなくて、用語の意味がわからないなら
> 、<a href="https://zombiebattleground.io/index.php/Gameplay" target=_blank>Zombie
> Battleground のゲームプレイルール</a>の簡単な紹介を読んでみましょう。

## `bytes serializedGameState`

`beforeMatchStart`は、`serializedGameState`という１つの引数`bytes`を受け取る。

これはバイトコードのチャンクであり扱いづらい。そしてこれは全プレイヤーのデッキ内
のカードやオーバーロードの体力、彼らの持つ Goo ボトルの数といったような **現在の
ゲーム状態全て** が含まれており、長い一行に凝縮したものだ。
...
この扱い辛そうなバイト配列を受け取ったら、これを扱いやすいフォーマットに変換する
のだが、それは次のチャプターでやっていこう。

データが人間が読めるフォーマットで受け渡しされずこのようなやり方になっているのは
、Zombie Battleground コアのゲームコードとカスタムゲームステートのスマートコント
ラクトの間で大量のデータをやりとりするのに、かなり効率の良い方法であるからなのだ
。

## さあテストだ

1. `beforeMatchStart`という名の`function`を定義せよ。これは１つのパラメーター
   ・`serializedGameState`という名の`bytes`を受け取る。この関数は`external`でな
   くてはならない。
