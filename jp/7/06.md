...
プレイヤーのスタート守備値や Goo ボトル数など、変数をうまく変更できたな！だがゲ
ーム内で変更できるのがこれだけだとしたら、カスタムゲームモードはかなり制限されて
しまうだろう。

このチャプターでは、カスタムゲームモードで可能な他のことを見ていこう。プレイヤー
のデッキ内のカードを変更するという、膨大なカスタマイズオプションが追加されるぞ。

これは、どのカードがカスタムゲームモードで使用できるかを制限すること(例:「Goo コ
スト３以下のカードのみ」)、もしくは個別のカードのスタッツを変更すること(例:お主
がパワー過剰/パワー不足だと思うカードの攻撃値や守備値、Goo コストを変更)、このど
ちらかの形式になる。

## 特定のカードを禁止

我らが`ExampleGame`カスタムゲームモード内で、**アイテムの使用を禁止** したいとし
よう。

これを行う最も簡単な方法は禁止したい全カードのリストを作って、各プレイヤーのデッ
キをループして各カードが禁止リストにないかをチェックし、もしあった場合はそのカー
ドを排除するというものだ。

> 注: 今後カスタムデッキ編集画面を追加して、ユーザーが前もってデッキを構築する際
> にあなたのゲームルールを実行するようにできる可能性があります。しかしカスタムゲ
> ームモードの機能の初期段階では、あなたのゲームルールを実行する唯一の方法
> は`beforeMatchStart()`の中で実行し、ルールでプレイされないカードは排除するとい
> うものです。
...
1. コントラクト冒頭で、`bannedCards`という`mapping (string => bool)`を宣言せよ。
   このマッピングは`internal`出なくてはならない。
...
次のチャプターでは各プレイヤーのデッキをループしながら、このマッピングを使って各
カードが禁止されていないかをチェックしていくからな。

> 注: 使用を許可されているカードよりも禁止されているカードの方が多いほどにあなた
> のモードがたくさんのカードを禁止している場合は、`bannedCards`ではなくマッピン
> グ`allowedCards`(許可されているカード)とする方が良いかもしれません。内部の実装
> はあなた次第となっており、この`mapping`はやり方の一例にすぎません。
