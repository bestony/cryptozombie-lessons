...
このチャプターでは、あの扱い辛いバイト配列`serializedGameState`を、簡単に読めて
やりとりできるものにデコンストラクトしていこう。

`ZBGameMode.sol`からアクセスできるヘルパー構造体の１つに`GameState`というものが
あり、これは`serializedGameState`にある全データを含んだデータ構造体である。

ではその中身を見ていこう。

## `GameState`

`GameState`はネスト化された`struct`であり現在の対戦の全状態、そして我々が変更す
ることを望みそうな全変数を含んでいる。 こんな感じだ:

> 注: こちらの各部分が何を行っているか理解することについてはご心配なく。ほぼ見て
> の通りなのですが、今後のチャプターでは現実的な例で１つ１つ噛み砕いていきます。
> こちらではトップレベルの概観をご確認ください。
...
手短にいうと: GameState は２人のプレイヤーを含み、各プレイヤーは一連のスタッツと
カードコレクションを持っている。

後ほどこの`GameState`構造体とやりとりする方法を見ていくが、まず初め
に`serializedGameState`のデータを使ってこの構造体を初期化していくぞ。
...
そのあと関数`init`を使って、`serializedGameState`の全データを読み込み
、`gameState`にそのデータを追加したい:

```
gameState.init(serializedGameState);
```

> 注: `init`の背後のロジックは`ZBSerializer.sol`の中にあり、やや複雑なものです。
> しかしこれは背後にあるものなので、カスタムゲームモードを作るために理解しようと
> 心配する必要はありません — あなたが超オタクでない限りは 😉
>
> 重要なことですが、Zombie Battleground のコアゲームロジックは`ZBSerializer`と全
> く同じエンコーディング/デコーディングフォーマットを使用しています。そのため T
> ゲームの２部分が通信し、ゲーム状態全体の受け渡しができるのです。

データが`gameState`に追加されたから、`gameState`のサブプロパティを使って現在のゲ
ーム状態にアクセスできるようになった。
