...
この関数はコード１行で定義することができるが、ではなぜ`if`ステートメントにインラ
インで定義せず、別途で関数を作成するのか？

その理由は、今後新たなゲームモードを作成する時このコントラクトをフォークして(こ
のコピーを作って)、そこをスタート地点とすることができるからだ。そうするとどのカ
ードがルールに則っているかを再定義する際に、この１つの関数の中身を変えるだけで良
いので非常にわかりやすくなるのだ。

## さあテスト�

1. `isLegalCard`という関数を定義せよ。この関数は`card`という名の`CardInstance`と
   いう１つの引数を受け取って`bool`を返却し、`internal`及び`view`というプロパテ
   ィを持つようにすること。

2. 関数本体では、`card.mouldName`が`bannedCards`マッピングにない場合に真を返却し
   なくてはならない。これは`return (!bannedCards[card.mouldName]);`で行うことが
   できる。
