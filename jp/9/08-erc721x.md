...
ユーザーが変換するのに十分なトークンを持っていることを検証できたから、今後は実際
に変換を行ってみるのだ。

この変換は２つの部分で行っていくからな:

1. ユーザーの`_tokenId`残高を減らす(他の誰に移転するわけではないから、事実上はそ
   のトークンを「破壊」している)。

2. 同量の NFT を生成し、同じユーザーに割り当てる。

## NFT の生成とは？

チャプター４で、マルチファンジブルトークンを生成するために`_mint()`関数を呼び出
したことを思い出すのだ。

ERC721XToken.sol には、`_mint()`関数のコピーが２つあるんだったな:
...
見ての通り、引数`_supply`を１つは受け取り、もう１つは受け取っていないのが違いと
なっている。

Solidity ではこのように、関数のバージョンを複数オーバーロードして、異なる数の引
数を受け取るようにすることが可能だ。

このため`ERC721XToken.sol`コントラクトにある全ての単一の関数をオーバーロードする
ことが可能だから、そこでは NT と NFT の両方に作用するバージョンがある —  だから
従来の ERC721 サービスとの後方互換性が可能になっているのだ。

つまりはユーザーがこの関数を呼び出してトークンを NFT に変換した後も、これらの
NFT は従来の ERC721 サービス全てで利用可能ということになるからな。

# さあテストだ

関数の実装を完了させよう。

1. 最初に`_updateTokenBalance`を呼び出し、`_tokenId`のユーザー残高か
   ら`_amount`分を差し引くのだ。`awardToken()`で使ったのと同じコード１行を使って
   できるからな。

2. 次に`for`ループを作成し、`uint i = 0`と宣言し`i < _amount`である限りループし
   続けるようにせよ。

3. `for`ループ内で`_mint`関数の NFT バージョンを呼び出すのだ。最初の引数に
   は`nftTokenIdIndex`を、２つ目の引数には`msg.sender`を渡せ。これでユニークな
   ID を持った NFT が生成され、同じユーザーへと割り当てられるようになるからな。

4. 次にカードがどの型のものかわかるように、この NFT の元々の`_tokenId`を保存する
   コードが必要だ。そこで`nftTokenIdToMouldId`マッピングで
   、`nftTokenIdIndex`→`_tokenId`とマップせよ。

5. 今度は`nftTokenIdIndex++`で nftTokenIdIndex をインクリメントせよ。これで次回
   NFT を作成するときに、ユニークな ID を持つことになる。

さあお終いだ！この関数は`_amount`分の FT をユーザー残高から差し引き、同数の NFT
を生成してユーザーに戻してくれるからな。

NFT バージョンの`_mint()`を呼び出す場合は、ユーザーが新たに受け取った NFT トーク
ンの ID を取得できるよう、コントラクトはイベントを送信することになる:
