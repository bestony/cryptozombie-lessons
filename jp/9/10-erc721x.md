...
もうほとんど出来上がっているからな！さらに１つの関数を作ってコントラクトを完成さ
せて、その後復習をしていこう。

最後に実装していくのは、トークンの **_バッチ生成_** である。この関数は１回の関数
コールで多くのトークンを生成することを可能にする —  そしてバッチ生成は、ERC721x
が Ethereum のガス代を節約することができる理由の１つなのだ。

これまで書いてきたコードのおかげで、この関数はかなり簡単に実装できるからな。

# さあテスト�

1. `mintToken`の直前で`batchMintTokens`という関数を宣言し、`external`か
   つ`onlyOwner`とせよ。これは`_tokenIds`という名の`uint[]`配列、及
   び`_tokenSupplies`という名の`uint[]`の２つの引数を受け取ること。

2. この関数は、`uint i = 0`から始まり`i < _tokenIds.length`の間継続する`for`ルー
   プを反復するようにせよ。

3. `for`ループの中では、シンプルに`_tokenIds[i]`と`_tokenSupplies[i]`の引数
   で`mintToken`を呼び出せ。

さあお終いだ！これで tokenId の配列と供給量を関数に渡し、大量のトークンを一度に
生成できるようになったな！すごく簡単だっただろ？
