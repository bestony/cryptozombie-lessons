...
よし！FT を NFT に変更して、ERC721 をサポートしているサービスとの互換性をもたせ
る関数が準備できたな！

だが、ユーザーが ERC721x の特徴を利用したサービスへとトークンを送り返したい場合
はどうなるのだろうか？

その場合は、反対に NFT を元の FT へと変換し直す関数が必要となる。

> 注: このレッスンの実装では、ユーザーがこれら関数を手動で呼び出すようにしていま
> す。しかし、例えばユーザーがサイドチェーンからメインネットへと(さらにその逆方
> 向に)トークンを送信する場合に NFT へ自動変換するといったように、違うやり方で実
> 装しても良いでしょう。

# さあテストだ

1. `convertToFT`という`public`な`function`を定義し、１つの引数`uint _tokenId`を
   受け取るようにせよ。

2. 最初に`tokenType`が`NFT`であるかをチェックする`require`ステートメントが必要だ
   。このやり方は`convertToNFT`でやったのと同じだからな。

3. 次に２つ目の`require`ステートメントを使って
   、`ownerOf(_tokenId)`が`msg.sender`に等しいことを確認せよ。等しくない場合は、
   エラー`"You do not own this token"`を出力すること。

4. `_updateTokenBalance`を呼び出して、呼び出し元の NFT 残高を`0`に設定したい。次
   の１行を使って実行せよ:
   `_updateTokenBalance(msg.sender, _tokenId, 0, ObjectLib.Operations.REPLACE);`

5. 次に`_updateTokenBalance`を呼び出して、この型のトークンの`msg.sender`の残高
   に`1`を`ADD`せよ。このトークンの mouldId は`nftTokenIdToMouldId[_tokenId]`で
   探すのだ。

6. 最後に、ユーザーに成功したことを知らせるようイベントを発行したい:
