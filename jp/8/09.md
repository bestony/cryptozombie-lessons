...
ネスト化された`for`ループが終わるまでに、全カードはランダムに`player1Cards`もし
くは`player2Cards`に追加されることになる。

だが最後のステップでは、各プレイヤーの`changes.changePlayerCardsInDeck`を呼び出
し、クライアントに渡す`changes`データへとこれらのカードをロードしなくてはならな
いのだ。

## さあテストだ

1.  _Singleton_ から残ったままの１行
    ・`changes.changePlayerCardsInDeck(Player(i), newCards, cardCount);`を切り取
    れ(またはコピーして削除せよ)。

2.  それを２つ目の`for`ループの **外側** にペーストせよ。このループはデッキが完
    全に構築される前に、**両方の** プレイヤーのループ実行を終了していなければな
    らない。

この１行は各プレイヤーに１回ずつ、計２回実行する必要がある。

a. １回目は、１つ目の引数に`Player(i)`ではなく`Player.Player1`を渡すこと。２つ目
と３つ目の引数は、`newCards`と`cardCount`の代わり
に`player1Cards`と`player1CardCount`を渡す。

b. このコード１行を再度実行するよう、コピー＆ペーストせよ。ただし今回はプレイヤ
ー２のための部分だから、プレイヤー１とされている全ての箇所をプレイヤー２に置き換
えること。
