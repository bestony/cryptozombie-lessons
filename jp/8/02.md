      ...
      "munchkin.sol": |
        ...
        contract Munchkin is ZBGameMode  {
            ...
            function beforeMatchStart(bytes serializedGameState) external {
                ...
                // 1. ネスト化された`for`ループをここにコピー＆ペーストするの�

                changes.emit();

            }

            // 2. isLegalCard()をここに定義するの�
...
コントラクトのシェルはできているから、今度はルールセットに基づいてカードをフィル
タリングするカスタムロジックを書いていこう。

`Munchkin`のルールに従うと、Goo コストが２より多いカードを除外するんだったな。

最初のステップとして、各プレイヤーのカードリストを反復しよう。前のレッスンで次
の`for`ループをやったことを思い出したな？:
...
これは各プレイヤーのデッキがカスタムゲームルールに適合しているかを検証するための
、共通のロジック構造みたいなものとなる。お主のカスタムゲームモードにいくらでもコ
ピー＆ペーストして使っていいからな。

## さあテスト�

1. 上のネスト化された`for`ループをコントラクト内にコピー&ペーストし、ルールに適
   合するカードのフィルタリング機能を追加できるようにせよ。

2. `isLegalCard`という名の関数を定義せよ。この関数は`card`という名の１つの引
   数`CardInstance`を受け取り、`bool`を返却する。関数のプロパティは`internal`及
   び`view`とすること。
