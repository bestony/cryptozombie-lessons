...
_Munchkin_ の最後のステップは、`isLegalCard()`の関数定義を埋めることだ。そしたら
このゲームモードの実装は完成するぞ！

かなり簡単だろ？

_Munchkin_ のルールは、 **Goo コストが２より多いカードを禁止** するものだったと
いうことを思い出すのだ。

`CardInstance`をチェックして、これらのカードをフィルタリングするベストな方法を考
えよう:
...
見ての通り、`gooCost` というプロパティがある。 _Munchkin_ においてカードがルール
に適しているかをテストするには、`card.gooCost`が２以下であるかチェックするだけで
良いのだ。

## さあテストだ

`isLegalCard()`は`bool`を返すから、この関数定義は`return`ステートメントのコード
一行で埋めることができる。

この関数は`card.gooCost`が２以下である場合は`true`、そうでなければ false を返却
しなくてはならない。
