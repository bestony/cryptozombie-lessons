...
> 2. `newCards`にすでに追加してある全カードをループし、そのカードがすでにその配
>    列にあるかをみる。すでに配列にあった場合は、`cardAlreadyInDeck`を`true`に設
>    定する。

あるカードがすでに配列にあるかを見る最も簡単な方法は、２枚のカード
の`mouldName`を比較することだ。同じカードを複数コピーしたものは、同
じ`mouldName`を持っているはずだからな。

> 注: mouldName とは、例えば"Zhampion"や"Burrrn"のようなカードの名前のことです
> 。displayName と区別するために`mouldName`と呼んでいます。なので開発者の皆さん
> は、単にカードの名前が表面的に変わったと思ってこのプロパティを変更することのな
> いようにしてください 🙃

しかし CryptoZombies のコース１を思い出してみると(もしくはお主が経験ある
Solidity 開発者であるなら) — Solidity には文字列を比較するネイティブなメソッドが
ないんだったな。 そういう訳で２つの文字列の`keccak256`ハッシュを比較して、それら
が同等であるかを見なくてはならないのだ:
...
1. `uint k`を 0 に等しく設定する`for`ループを宣言せよ。そして`k < cardCount`であ
   る限りは繰り返すようにすること。

2. `for`ループの中に、上にあるよう
   に`newCards[k].mouldName`と`gameState.playerStates[i].cardsInDeck[j].mouldName`の`keccak256`ハ
   ッシュが同等であるかを比較する`if`ステートメントを作るのだ。

3. 条件が真であれば、`cardAlreadyInDeck = true;`と設定するようにせよ。

さあ出来た！この完成したループで、カードが`newCards`にあれ
ば`cardAlreadyInDeck`は`true`になり、なかった場合は`false`になる。そして前のチャ
プターで実装したロジックは、`cardAlreadyInDeck`が`false`の場合のみカードをデッキ
に加えるようになっている。

これで Singleton!のロジックは完成だ！
