...
これでカード毎のランダムなコイントスを実装して、その結果次第でコードがどちらかの
プレイヤーのデッキにカードを追加するようにできたな。

だが１つ問題があるのだ —  コイントスの結果、もしプレイヤー１に半数以上のカードが
配られてしまったら...？

これはゲームにとって不公平であるだけでなく、我々が作成したプレイヤーのカード配列
をオーバーフローさせてしまうことになる 🙃

だからこういうことが起こらないよう、しっかりとコードでチェックする必要があるの�
。

## さあテスト�

`if`ステートメントの前に **別の**`if`ステートメントを追加して、配列がオーバーフ
ローしてしまわないかをチェックしたい。

1. `player1CardCount + 1`が`gameState.playerStates[0].cardsInDeck.length`よりも
   大きいことをチェックする`if`ステートメントを追加せよ。

2. 真である場合は、`rand = 1`に設定せよ(こうして、次の if/else ステートメントは
   プレイヤー１ではなくプレイヤー２にカードを与えることとなる)。

3. `else if`節を追加し
   、`player2CardCount + 1`が`gameState.playerStates[1].cardsInDeck.length`より
   大きいことをチェックせよ。
