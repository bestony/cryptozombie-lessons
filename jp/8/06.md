      ...
      "zombiepile.sol": |
        ...
        contract Singleton is ZBGameMode  {
            ...
            function beforeMatchStart(bytes serializedGameState) external {
                ...
                //プレイヤー２のcardCount変数を宣言するように変更するの�
...
よく出来た！_Singleton_ はこれで完成だ。カスタムデッキルールを持った２つ目のゲー
ムモードを実装出来たな。

では今度は３つ目のゲームモード・ _Zombie Pile_ を実装していこう。また前のコント
ラクトをフォークして`zombiepile.sol`という名前の新規ファイルを作っておいたから、
このファイルから始められるぞ。だが`singleton.sol`のタブを参照することもできるよ
うにしてあるからな。

このゲームモードのルールを思い出そう:

> **Zombie Pile**: バトルのスタート時に、両プレイヤーのカードが合わせてシャッフ
> ルされ、両者は自分と相手のカードをミックスしたデッキをランダムに受け取る。

このロジックの実装方法は、デッキをまとめてシャッフルし、次のステップで全カードを
配るのとは実際少し異なっている。

そうする代わりに、両プレイヤーのデッキのカードを１枚ずつ反復すると、１回のステッ
プでこれを行うことが出来るのだ(前の実装で行ったように)。そしてカード毎に、コイン
トス(0 か 1 の数字をランダムに生成)を行う。数字が`0`の場合、プレイヤー１のデッキ
にカードを追加し、`1`の場合はプレイヤー２のデッキに追加する。

こうすることで、デッキを合わせてシャッフルするのと同じ効果になるが、より簡単にコ
ードで概念化できるのだ。

このロジックはこれまでの実行とは少し違っているから、`for`ループ _以前で_ プレイ
ヤー１とプレイヤー２の`newCards`をインスタンス化する必要がある。こうして、ループ
反復中に各カードを２つのデッキのうちの１つに追加することができ、プレイヤー毎に１
度に１枚のカードでこれをやる必要がなくなるのだ。

## さあテスト�

1. まず最初に、このコントラクト名を`ZombiePile`に変更せよ。

2. `CardInstance[] memory newCards newCards`と`uint cardCount`を宣言している２行
   を for ループ以前に移動させなければならない。このコード２行を切り取ること(も
   しくはコピーしてから削除せよ)。

3. その２行を`for`ループの直前にペーストせよ。そして今度はこれを編集して、１つで
   はなく２つのデッキを宣言するようにしなくてはならない。

a. `newCards`ではなく、２つの配列`player1Cards`及び`player2Cards`と宣言しよう。

b. `player1Cards`の長さは`gameState.playerStates[0].cardsInDeck.length`でなけれ
ばならず、`player2Cards`の長さは`gameState.playerStates[1].cardsInDeck.length`で
なくてはならない。

> 注: これは各プレイヤーがゲーム開始時と同じカードの枚数を受け取ることになるので
> 、シャッフルと全く同じことではありません。しかしこれは、例えば 30 枚ではなく
> 15 枚か 16 枚のカードをプレイヤーが受け取るような場合の対戦に、１枚のカードを
> 持ち込んでプレイヤーが対戦相手を欺くのを妨げることができます。

4. `player1Cards`と`player2Cards`の両方を宣言したら、２つの`uint`も宣言しなくて
   はならない: `player1CardCount`と`player2CardCount`だ。これら両方を`0`と同等に
   設定すること(これは変数`cardCount`を置き換える)。
