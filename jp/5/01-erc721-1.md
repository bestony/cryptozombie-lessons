...
`balanceOf(address _owner)` といった関数のスタンダードセットを実装しているもの�
。
...
この一例として挙げられるのは取引所であろう。取引所が新たな ERC20 トークンを追�
するのに必要なのは、トークンとやり取りするためのスマート・コントラクトをただ追�
...
💯💯😎💯💯**"と同じものだなんてことは全くない！(_Steve_ よ、わしのゾンビとは程�
いぞ)
...
> 注: ERC721 のような標準を使用するメリットとして、オークション及びプレイヤーが
