...
By the end of that nested `for` loop, every card will have been randomly added
to either `player1Cards` or `player2Cards`.

But the last step is, we need to call `changes.changePlayerCardsInDeck` for each
player to actually load these cards into the `changes` data that we'll pass back
to the client.

## Put It to the Test

1. Cut (or copy then delete) the line
   `changes.changePlayerCardsInDeck(Player(i), newCards, cardCount);` that was
   left over from _Singleton_.

2. Paste it **outside** the second `for` loop. That loop needs to finish running
   for **both** players before our decks are completely constructed.

We need to execute this line of code twice, once per each player.

a. The first time, instead of `Player(i)`, it should pass `Player.Player1` for
the first argument. And the 2nd and 3rd arguments should pass `player1Cards` and
`player1CardCount` instead of `newCards` and `cardCount`.

b. Copy/paste this line of code to run it again, but this time for Player 2
— swap out all references to Player1 with Player2.
