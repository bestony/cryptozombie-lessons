...
> 2. Loop through every card in `newCards` to see if the card is already there.
>    If it is, set `cardAlreadyInDeck` to `true`

The easiest way to see if the card is already there is to compare `mouldName`s
of the two cards — multiple copies of the same card will have the same
`mouldName`.

> Note: A mouldName is the name of the card, for instance "Zhampion" or
> "Burrrn". We call it `mouldName` to distinguish it from a displayName, so
> developers don't change this property thinking they're simply changing the
> cosmetic name of the card 🙃

However, if you'll recall from CryptoZombies Course 1 (or if you're an
experience Solidity developer) — Solidity has no native methods to compare
strings. Thus we must compare the `keccak256` hashes of the two strings to see
if they're equal:
...
1. Declare a `for` loop that sets `uint k` equal to 0, and iterates as long as
   `k < cardCount`.

2. Inside the `for` loop, create an `if` statement that compares the `keccak256`
   hashes of `newCards[k].mouldName` and
   `gameState.playerStates[i].cardsInDeck[j].mouldName` are equal, as above.

3. If the statement is true, then set `cardAlreadyInDeck = true;`

And we're done! After this loop is complete, `cardAlreadyInDeck` will be `true`
if it was found in `newCards`, and `false` if it was not found. And the logic we
implemented in the last chapter will add it to the deck only if
`cardAlreadyInDeck` is `false`.
