...
Now we've implemented a random coin flip for each card, and the code will add it
to one of the player's decks depending on the outcome of that coin flip.

But there's a problem — what if the coin flip gives Player 1 MORE than half the
cards?

This not only wouldn't be fair to the game — but it would overflow the array we
created to hold his cards 🙃

So we need to add a check in our code to make sure this doesn't happen.

## Put It to the Test

Before our `if` statement, let's add **another** `if` statement that checks to
see whether we're about to overflow the array.

1. Add an `if` statement that checks if `player1CardCount + 1` is greater than
   `gameState.playerStates[0].cardsInDeck.length`

2. If it's true, set `rand = 1` (this way our following if/else statement will
   give the card to Player 2 instead of Player 1).

3. Add an `else if` clause that checks if `player2CardCount + 1` is greater than
   `gameState.playerStates[1].cardsInDeck.length`.
