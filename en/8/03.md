...
The last step for _Munchkin_ is to fill out the function definition for
`isLegalCard()` — then this game mode will be fully implemented!

Super simple, right?

Recall from the rules of _Munchkin_ that we want to **ban any card with a goo
cost more than 2**.

To see the best way to filter these cards, let's take another look at
`CardInstance`:
...
As you can see, we have a `gooCost` property. So to test whether a card is legal
in _Munchkin_, we can simply check whether `card.gooCost` is less than or equal
to 2.

## Put It to the Test

Since `isLegalCard()` returns a `bool`, this function definition can be filled
out with one line of code — a `return` statement.

The function should return `true` if `card.gooCost` is less than or equal to 2,
and false otherwise.
