...
Congratulations! That completes the logic for _Singleton_. You've now
implemented your second mode with custom deck rules.

Now we're going to implement the third game mode, _Zombie Pile_. Again, we've
forked the previous contract into a new file called `zombiepile.sol` as a
starting point, but you can still reference `singleton.sol` in its own tab.

As a refresher on the rules of this game mode:

> **Zombie Pile**: At the start of each match, both players' cards are shuffled
> together, and they each randomly get a mix of their own deck and their
> opponents deck.

The way we're going to implement this logic is a bit different than actually
shuffling the decks together and then dealing all the cards out as a secondary
step.

Instead, we're going to do it in one step by iterate through both players decks
card by card (as we did in the previous implementations). And for each card,
we're going to flip a coin (generate a random number that's either 0 or 1). If
the number is `0`, we'll add the card to Player 1's deck. If it's `1`, we'll add
it to Player 2's deck.

This will have the same effect as shuffling the decks together would, but is
easier to conceptualize in code.

Since this logic is a bit different from the previous implementation, we'll need
to instantiate `newCards` for both Player 1 and Player 2 _before_ the `for`
loop. That way we can add each card to one of the two decks within each loop
iteration, instead of doing them one at a time for each player.

## Put It to the Test

1. First of all, change the name of this contract to `ZombiePile`

2. We need to move the lines of code where we declared
   `CardInstance[] memory newCards newCards` and `uint cardCount` to before the
   for loop. Cut these 2 lines of code (or copy and then delete them).

3. Paste those lines right before the `for` loop. Now we need to modify them to
   declare 2 decks instead of 1.

a. Instead of calling it `newCards`, let's call the 2 arrays `player1Cards` and
`player2Cards`.

b. The length of `player1Cards` should be
`gameState.playerStates[0].cardsInDeck.length`, and the length of `player2Cards`
should be `gameState.playerStates[1].cardsInDeck.length`.

> Note: This isn't the exact same as a shuffle, since each player will receive
> the same number of cards back as they entered the game with. But this prevents
> a player from trying to trick their opponent, say by only bringing 1 card into
> the match such that each player ends up with 15 or 16 cards instead of 30.

4. After we declare both `player1Cards` and `player2Cards`, we need to declare 2
   `uint`s: `player1CardCount` and `player2CardCount`. Set them both equal to
   `0`. (These will replace the `cardCount` variable).
