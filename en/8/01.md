...
In this lesson, we're going to do a rapid-fire look at implementing **three
different custom game modes**.

Looking at a number of examples in succession should hopefully get you pretty
comfortable with the process of creating a game mode with custom deck rules.

Here are the three modes we will be implementing, and a quick summary. _Thanks
to one of our community members, Brooks Boyd (aka MidnightLightning), for these
game mode ideas._

1. **Munchkin**: When assembling your deck of 30 cards to play, you may only
   pick from _cards that have a cost of two goo or less._

2. **Singleton**: When assembling your deck of 30 cards to play, _no card may
   share a name with another card in the deck._ (I.e. You may only have one of
   each card in your deck).

3. **Zombie Pile**: At the start of each match, _both players' cards are
   shuffled together_, and they each randomly get a mix of their own deck and
   their opponents deck.

Let's start this lesson off by implementing the first mode: **Munchkin**.

## Getting Started

We'll start with a (mostly) empty shell of a contract that contains some of the
core logic we learned in the previous lesson.

Typically, when you set out to create a new custom game mode, you'll want to
fork / copy an existing contract and modify it from there. This is a bare-bones
contract you can use as a template in the future if you want to build from
scratch. After you complete this lesson, you can use any of these 3 game modes
as a starting template for your own game mode.

First we'll want to give our contract a new name, and in the next chapter we'll
start on our game mode's custom logic.
