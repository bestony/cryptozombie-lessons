...
Great! Now you've completed your _Munchkin_ game mode smart contract!

Making these custom game modes is super simple, right?

Now let's move on to the next one:

_**Singleton:** When assembling your deck of 30 cards to play, no card may share
a name with another card in the deck._

To start with, we've forked (made a copy of) `munchkin.sol` and called it
`singleton.sol`.

Again, this is the easiest way to start off a new custom game mode — fork a
previous contract and modify it, rather than starting from scratch.

> Note: We've also left a `munchkin.sol` tab in the editor, in case you want to
> go back and reference the finished contract at any point throughout this
> lesson.

_Singleton_ is a bit trickier to implement than _Munchkin_ was. With _Munchkin_,
we had a simple criteria we could evaluate every card against in
`isLegalCard()`.

However, with _Singleton_, the ruleset to determine if a card is legal is to
compare it with every other card we've already added to the `newCards` array
— we'll need another `for` loop to do this.

The logic will look something like, for each card...

1. Declare a `bool` named `cardAlreadyInDeck` and set it to `false`

2. Loop through every card we've already added to `newCards` to see if the card
   is already there. If it is, set `cardAlreadyInDeck` to `true`

3. At the end of this loop, we'll check if `cardAlreadyInDeck` is still false —
   and if it is, we'll add this card to `newCards`.

We'll make these changes in this chapter and the next.

## Put It to the Test

To start with, let's rename this new contract, and start making changes we
described above.
...
3. We'll implement the third `for` loop in Step 2. above in the next chapter.
   For now, let's implement Step 3:

Modify the condition of our existing `if` statement to check if
`cardAlreadyInDeck` is still false. You can do this with `!cardAlreadyInDeck`.

4. Lastly, delete the entire function `isLegalCard()`, since we won't need it in
