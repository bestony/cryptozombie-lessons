...
Now that we have a shell for our contract, we want to write the custom logic
that filters players' cards based on a set of rules.

According to the rules of `Munchkin`, we want to filter out any card that costs
more than 2 goo.

The first step is to iterate through each player's cards, and you'll recall from
the previous lesson that we did that with the following nested `for` loop:
...
This will be such a common logic structure in your custom game modes for
verifying whether each player's deck is legal according to your custom game's
rules, that you'll want to copy & paste this logic into a lot of your custom
game modes.

## Put It to the Test

1. Copy & paste the above nested `for` loop into your contract, so we can add a
   filter for legal cards.

2. Define the function named `isLegalCard`. This function will receive one
   argument, a `CardInstance` named `card`, and return a `bool`. The function
   should have the properties `internal` and `view`.
