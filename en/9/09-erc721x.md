...
Great! Now we have a function that takes some FTs and converts them to NFTs that
can be compatible with anything that supports ERC721.

But what about if the user wants to send them back to a service that takes
advantage of the ERC721x features?

In that case, we'll need a function that does the opposite — converts from an
NFT back to its original FT.

> Note: In the implementation in this lesson, we're making it so the user calls
> these functions manually. But you could do your implementation differently
> — for instance, auto converting to an NFT if the user sends a token from the
> sidechain to mainnet, and vice versa.

# Put It to the test

1. Define a `function` called `convertToFT` that takes one argument
   — `uint _tokenId` — and is `public`.

2. First, we need a `require` statement that checks to make sure the `tokenType`
   is an `NFT`, similar to how we did it in `convertToNFT`.

3. Next, we'll use a second `require` statement to make sure `ownerOf(_tokenId)`
   is equal to `msg.sender`. Otherwise, it should output the error
   `"You do not own this token"`

4. We'll want to call `_updateTokenBalance` to set the caller's balance of the
   NFT to `0`. We can do this with the following line:
   `_updateTokenBalance(msg.sender, _tokenId, 0, ObjectLib.Operations.REPLACE);`

5. Next, call `_updateTokenBalance` to `ADD` `1` to `msg.sender`'s balance of
   the mould for this token. You can look up this token's mouldId with
   `nftTokenIdToMouldId[_tokenId]`.

6. Lastly, we'll want to emit an event to let the user know this was successful:
