...
The `GameState` we initialized in Chapter 3 can now be used to read data from
the current game state.

At the end of `beforeMatchStart()`, we’ll need to pass the data back to the game
server as a `SerializedGameState`.

In order to do this, we need to declare a custom data type named
`ZBSerializer.SerializedGameStateChanges`, which will track all the changes we
make.
...
`changes` is, as the name implies, where we will make all the changes to the
game state that we will return at the end of this function.
...
This gets it ready to start storing data. Unlike with `gameState`,
`changes.init()` does not require any arguments.

## Put It to the Test

1. Declare a `ZBSerializer.SerializedGameStateChanges` in `memory` called
   `changes`.
