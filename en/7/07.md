...
Now that we’ve defined a list of the cards that are banned in our game mode, we
have to write the logic to check the cards in each player’s deck and remove the
ones that are not allowed.

Since it’s not simple to delete an element in an array in Solidity, it’s easier
to simply create a new array in memory, and copy all the “allowed” cards over.

Let’s walk through the pieces we need to do this.

## Iterating through a list of players

Recall that our `gameState` struct now contains all the details of the current
game state, including both players and their cards.
...
To make a new array of cards for each player, so we can copy the allowed cards
over one by one, we can do the following:
...
2. For each player, create an array of `CardInstance`s called `newCards`. The
   length of this array should be `gameState.playerStates[i].cardsInDeck.length`
