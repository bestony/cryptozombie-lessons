...
This function can be defined with one line of code. So why create a separate
function, and not just define it inline in the `if` statement?

This is because in the future when making a new game mode, you can fork this
contract (make a copy of it) and use it as a starting point. And when you do so,
it will be totally clear that you only need to change the contents of that one
function to redefine the ruleset for which cards are legal.

## Put It to the Test

1. Define a function called `isLegalCard`. This function will receive one
   argument, a `CardInstance` named `card`, and return a `bool`. The function
   should have the properties `internal` and `view`.

2. In the function body, it should return true if the `card.mouldName` is NOT in
   the `bannedCards` mapping. You can do this with
   `return (!bannedCards[card.mouldName]);`.
