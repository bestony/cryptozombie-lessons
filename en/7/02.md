...
The shell of a Zombie Battleground custom game mode contract starts like any
other smart contract in Solidity, which you can see to the right.

The first thing to note is that we’re importing from a file named
`ZBGameMode.sol`, which imports from `ZBSerializer.sol` and a few other files.

> Note: Some of these other files are quite large, so we're not including them
> in the lesson. You can access the full source on
> <a href="https://github.com/loomnetwork/zb_game_mode" target=_blank>this
> github repo</a>. But you don't need to worry about reading/understanding the
> contents of these files — we'll be explaining the relevant portions that you
> need to know in order to create your custom game mode.
...
make your life easier as a custom game mode developer.

We've abstracted these into a library, so you won't need to put the same
boilerplate logic in every custom game mode.

### "Hooks" — the heart of every custom game mode

The first step for every custom game mode is to implement at least one
**_hook_**.

Basically, the core game logic will look for and call certain functions from
your contract at certain parts of the game, if they exist in your code.

For example, `beforeMatchStart` will be called just before the match
starts; `afterInitialDraw` will be called immediately after players draw their
first cards; `beforeTurnStart` _(planned but not implemented yet)_ will fire at
the start of each player's turn.

These hooks allow you to “tap into” and modify the game state at that point,
applying your custom rules before the game continues.

The first hook we'll implement is called `beforeMatchStart`, which is a hook
that is applied — as its name implies — before the start of every match.
...
Inside the function, you will define all the custom changes to the game logic
you want to be applied at the start of a match.

These changes could be things like balance changes to individual cards, rules
for which cards are allowed/banned in this game mode, and changes to overlord
health or the starting number of goo vials.

> Never played Zombie Battleground and not sure what any of these terms mean?
> Read a quick intro to
> <a href="https://zombiebattleground.io/index.php/Gameplay" target=_blank>the
> gameplay rules of Zombie Battleground</a>.

## `bytes serializedGameState`

`beforeMatchStart` accepts one argument — a `bytes` called
`serializedGameState`.

This is a nasty chunk of bytecode that contains the **entire current state of
the game** — all the players' cards in their decks, their overlord healths,
number of goo vials they have, etc. — condensed into a single long string.
...
After we receive this nasty-looking byte array, we’ll convert it into a format
that’s easy to work with — which we’ll look at in the next chapters.

The reason it’s done this way as opposed to passing the data in human-readable
format, is because this is a far more efficient way to pass large amounts of
data back and forth between the Zombie Battleground core game code and the
custom game state smart contract.

## Putting It to the Test

1. Define a `function` named `beforeMatchStart`. It will take one parameter, a
   `bytes` named `serializedGameState`. This function should be `external`.

2. Leave the function body empty for now — we will fill it in in the next
   chapter.
