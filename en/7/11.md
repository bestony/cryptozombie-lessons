...
After our nested `for` loop finishes running, we will have swapped out both
players’ cards with a new set that doesn’t include any banned cards. (If the
player had no items in their deck from the get go, no changes will have been
made).

We’ve also made some changes to each player’s starting defense and goo vials.

But recall that `changes` is simply a local data type stored in memory. To
complete our game mode, we’ll need to pass this data back to the core game
logic.
...
As the name describes, this will emit an `event`, which the game core logic is
listening for.

This event contains the data of all the changes we want to make, in a format
similar to `serializedGameState`.
