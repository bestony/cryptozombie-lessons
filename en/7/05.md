...
`changes` has a bunch of helper functions we can use in order to modify specific
parts of the game state.

For example, if you wanted to change Player 1’s Overlord starting defense stat
from 20 to 30, you would do the following:
...
For those who aren’t familiar, in Solidity, an `enum` is a way of creating a
user-defined data type that’s easily convertible to and from a `uint`.
...
Why would we want to do this? Mainly for code readability, and better clarity on
...
In the first example, it’s easy to see at a glance that we’re modifying the stat
for Player 1, whereas in the second example, someone reading the code would need
to look up the function definition to see what the `0` refers to.

## Changing Zombie Battleground player stats

In addition to `changes.changePlayerDefense()` above, there are a number of
other methods we can call on the `changes` object to tweak different stats in
the game:

- `changePlayerCurrentGooVials` — the number of vials the player currently has
  available for this round. The default starts at 1 when the match starts, and
  increases by 1 with every turn.
- `changePlayerCurrentGoo` — the number of vials that are filled with goo in the
  current round. Defaults to be the same as the number of goo vials at the start
  of each turn.
- `changePlayerMaxGooVials` — the max number of goo vials a player can have in
  the game. Default is 10.
- `changePlayerInitialCardsInHandCount` — the number of cards the player is
  dealt when they start the match. Default is 3.
- `changePlayerMaxCardsInPlay` — the maximum number of zombies a player can
  place on the field. Default is 6.
- `changePlayerMaxCardsInHand` — the maximum number of cards a player can hold
  in their hand at one time. Default is 10

These functions all follow the same format as `changePlayerDefense` and accept
two arguments: A `Player` enum, and a `uint8`.

## Put It to the Test

Let’s create some custom game rules for our ExampleGame mode by tweaking some of
these values.

1. Set Player1 and Player2’s starting defense to 15
2. Set `CurrentGooVials` for both players to 3, and also set `CurrentGoo` for
   both players to 3.
3. Set `MaxGooVials` for both players to 8

The above should be 8 lines of code in total. You can find the method names
above, and recall that all methods follow the same format as
`changes.changePlayerDefense(Player.Player1, 30);`
