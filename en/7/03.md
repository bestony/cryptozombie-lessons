...
In this chapter, we’re going to deconstruct that nasty byte array,
`serializedGameState`, into something we can easily read and interact with.

One of the helper structures we have access from `ZBGameMode.sol` is called
`GameState` — and is a data structure that contains all the data within
`serializedGameState`.
...
`GameState` is a nested `struct` that contains the entire state of the current
match — and all the variables we might want to change. It looks like this:

> Note: Don’t worry about understanding what every piece of this does just yet —
> although most of them are self-explanatory. We’ll break these down one by one
> with real world examples in future chapters. This is just to give you a
> top-level overview.
...
In short: A GameState contains 2 Players, and each Player has a bunch of stats
and a collection of Cards.

Later we’ll look at how to interact with this `GameState` struct. But first
let’s initialize it with the data from `serializedGameState`.
...
Then we want to use the function `init` to read in all the data from
`serializedGameState` and use it to populate our `gameState` with data:

```
gameState.init(serializedGameState);
```

> Note: The logic behind `init` is in `ZBSerializer.sol`, and is a bit
> complicated. But this is behind the scenes, and you don’t need to worry about
> understanding it in order to make a custom game mode — unless you're super
> nerdy like that 😉
>
> The important part is that the core Zombie Battleground game logic is using
> the exact same encoding and decoding format as `ZBSerializer`, so the two
> parts of the game are able to communicate and pass an entire game state back
> and forth.

Now the data is populated in `gameState`, and we’ll be able to access the
current state of the game using `gameState`’s sub-properties.

For example, if you wanted to see what the name of the first card in Player 1’s
deck is, you could do so with:
