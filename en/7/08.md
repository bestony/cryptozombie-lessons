...
Continuing from the last chapter, we now want to iterate through each card in
the player’s deck and check to see if they’re banned.

## Looping through the cards in a player’s deck

Here’s an example of how you would iterate through each card in both players’
decks, and change them all to “Slab” — a fan favorite card 😉
...
As you can see, we use a nested `for` loop to first iterate through the list of
each player, then for each player iterate through a list of each of his cards.

> Note: Changing a `mouldName` of a card in a player’s deck will actually swap
> that card out entirely with the card named Slab. It’s not just a display name.

## Put It to the Test

1. Create a second `for` loop that iterates through all the `cardsInDeck` for
   each player, as above

2. Create an `if` statement that calls a function `isLegalCard` and passes it
   `gameState.playerStates[i].cardsInDeck[j]`. We will define this function in
   the next chapter.

3. Inside the `if` statement, copy the card over to our new array by setting
   `newCards[cardCount]` equal to `gameState.playerStates[i].cardsInDeck[j]`.
