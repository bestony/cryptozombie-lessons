...
Changing variables like the player’s starting defense and goo vials is great.
But if that’s the ONLY thing we could change in the game, custom game modes
would be really limited.

In this chapter, we’ll take a look at something else we can do in custom game
modes that adds an enormous amount of customization options: modifying the cards
in a player’s deck.

This could come in two forms — either by limiting which cards are allowed to be
used in your custom game mode (e.g. “only cards that cost less than 3 goo”), or
by changing the stats of individual cards (e.g. chainging the attack, defense,
or goo cost cards you think are over- or under-powered).

## Banning specific cards

Let’s say in our `ExampleGame` custom game mode, we wanted to **ban the use of
items.**

The simplest way to do this is by making a list of all the cards we want to ban,
then looping through each player’s deck to check if each card is on the banned
list and removing the card if it is.

> Note: Later we’ll probably add custom deck editing screen where you can
> enforce your game rules as the users build their decks ahead of time. But in
> the early stages of our custom game mode functionality, the only way to
> enforce it is in `beforeMatchStart()`, and removing the cards that don't play
> by the rules.
...
1. At the top of our contract, declare a `mapping (string => bool)` called
   `bannedCards`. This mapping should be `internal`.
...
3. Inside the constructor, declare which cards are banned with the following
   code:
...
In the next chapter, we’ll use this mapping to check each card to see if it’s
banned, as we loop through each player’s deck.

> Note: If your mode bans so many cards that there are more banned cards than
> allowed cards, it would probably be better to implement this as an
> `allowedCards` mapping, rather than `bannedCards`. The internal implementation
> is up to you, and this `mapping` is just one example of how you could do it.
