...
查看卡牌是否已经存在，最简单的方法是比较两张卡牌的`mouldName` —— 同一张卡牌的多
个副本将具有相同的`mouldName`。

> 注意：`mouldName`是卡牌的名称，例如“Zhampion”或“Burrrn”。 我们将其命名
> 为`mouldName`以便同 displayName 区分，这样开发人员就不会更改此属性，因为他们只
> 是更改了卡牌的外观名称 🙃

但是，如果你回忆下 CryptoZombies 第 1 季课程（或者如果你是经验丰富的 Solidity 开
发人员）—— Solidity 没有原生方法来比较字符串。因此，我们必须比较两个字符串
的`keccak256`哈希值，看看它们是否相等：
...
1. 声明一个`for`循环，将`uint k`设置为 0，并且只要`k < cardCount`就迭代。

2. 在`for`循环中，创建一个`if`语句，用于比
   较`newCards[k].mouldName`和`gameState.playerStates[i].cardsInDeck[j].mouldName`的`keccak256`哈
   希值，如上所述。

3. 如果该语句为 true，则设置`cardAlreadyInDeck = true;`

完成啦！ 此循环结束后，如果在`newCards`中找到`cardAlreadyInDeck`，则为`true`；如
果未找到，则为`false`。 只有当`cardAlreadyInDeck`为`false`时，我们在上一章中实现
的逻辑才会将它添加到牌组中。

这样就完成了 Singleton 的逻辑！
