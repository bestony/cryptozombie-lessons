...
现在我们已经为每张卡牌实现了随机抛一次硬币，代码将根据抛掷结果将该卡牌添加到其中
一个玩家的牌组中。

但是有个问题 —— 如果抛硬币的结果让 1 号玩家获得的超过一半的卡牌，怎么办？
...
1. 添加一个`if`语句，检查`player1CardCount + 1`是否大
   于`gameState.playerStates[0].cardsInDeck.length`。

2. 如果是，则设置`rand = 1`（这样我们后面的 if / else 语句将把卡牌给 2 号玩家而
   不是 1 号玩家）。

3. 添加`else if`子句，检查`player2CardCount + 1`是否大
   于`gameState.playerStates[1].cardsInDeck.length`。

4. 如果是，则设置`rand = 0`（将卡牌给 1 号玩家）。
