...
*Munchkin*的最后一步是填充`isLegalCard()`的函数定义 —— 然后这个游戏模式就完全实
现了！

超级简单，对吧？

回想一下*Munchkin*的规则，我们要**禁止任何 goo 值超过 2 的卡牌**。
...
如你所见，我们有一个`gooCost`属性。 因此，要测试一张卡牌在*Munchkin*中是否合法，
我们可以直接检查`card.gooCost`是否小于或等于 2。

## 实战演�

由于`isLegalCard()`返回`bool`，因此可以使用一行代码填充此函数定义 —— `return`语
句。

如果`card.gooCost`小于或等于 2，该函数应返回`true`，反之则返回`false`。
