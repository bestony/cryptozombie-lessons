...
但最后一步是，我们需要为每个玩家调用`changes.changePlayerCardsInDeck`将这些卡牌
真正加载到我们将传递回客户端的`changes`数据中。

## 实战演�

1. 剪切（或复制然后删除）从*Singleton*留下来的
   行`changes.changePlayerCardsInDeck(Player(i), newCards, cardCount);`。

2. 将其粘贴在第二个`for`循环**外**。 在我们的牌组完全构建好之前，该循环需要
   为**两个**玩家都完成运行。

   我们需要将这行代码执行两次，每个玩家一次。

   a. 第一次，它应该传递`Player.Player1` 作为第一个参数，而不是`Player(i)`。 第
   二和第三个参数应该传递`player1Cards` 和`player1CardCount`，而不是`newCards`
   和`cardCount`。

   b. 复制/粘贴这行代码再次运行它，但这次是 2 号玩家 —— 用 Player2 替换所有对
   Player1 的引用。
