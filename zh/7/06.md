...
更改像玩家初始防御值和 goo 瓶数这样的变量是很好。但是如果这是我们在游戏中唯一能
够改变的东西，那么自定义游戏模式的局限性就很大了。

在本章中，我们将看看我们在自定义游戏模式中可以做的另一件事情，它增加了大量的自定
义选项：修改玩家牌组中的卡牌。

有两种形式，一是通过限制允许在你的自定义游戏模式中使用哪些卡牌（例如“只有 goo 成
本低于 3 的卡牌”），二是通过更改单张卡牌的数据（例如，改变你认为攻击、 防御、或
者 goo 成本过高或过低的卡牌）。
...
最简单的方法是列一个我们想要禁用的所有卡牌的清单，然后循环访问每个玩家的牌组，检
查每张卡牌是否在禁用名单上，如果是，则移除该卡牌。

> 注意：稍后我们可能会添加自定义牌组编辑屏幕，你可以在用户构筑牌组之前强制执行你
> 的游戏规则。但是在我们的自定义游戏模式功能的早期阶段，强制执行它的唯一方法是
> 在`beforeMatchStart（）`中，然后移除不符合规则的卡牌。

## 实战演习

让我们给我们的自定义游戏模式添加一个禁用卡牌清单。

1. 在我们合约顶部，声明一个名为`bannedCards`的`mapping (string => bool)` 。这个
   映射应该是`internal`。
...
在下一章中，我们将用这个映射循环访问每个玩家的牌组，来检查每张卡牌以确定它是否被
禁用。

> 注意：如果你的模式禁用了太多卡牌，导致禁用的卡牌比允许使用的卡牌更多，那么最好
> 将它作为`allowedCards`映射来实现，而不是`bannedCards`。内部实现取决于你的选择
> ，这个`mapping`只是你能如何实现的一个例子。
