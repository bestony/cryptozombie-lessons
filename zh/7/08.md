...
以下是一个例子说明如何在两个玩家的牌组中遍历每张卡牌，并将它们全部更改为“Slab” -
一张粉丝们最爱的卡牌 😉
...
正如你所见，我们使用嵌套`for`循环首先遍历列表中的每个玩家，然后再遍历每个玩家的
每张卡牌。

> 注意：更改玩家牌组中卡牌的`mouldName`实际上会将该卡牌完全换成名为 Slab 的牌。
> 它不仅仅是一个显示名称。
...
2. 创建一个调用`isLegalCard`函数的`if`语句，并给它传
   递`gameState.playerStates[i].cardsInDeck[j]`。我们将在下一章定义这个函数。

3. 在这个`if`语句中，通过设置`newCards [cardCount]`等
   于`gameState.playerStates [i] .cardsInDeck [j]`将卡牌复制到我们的新数组。
