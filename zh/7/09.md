...
这个函数可以用一行代码来定义。那么为什么要创建一个单独的函数，而不只是在`if`语句
中内联它？

这是因为将来在制作新游戏模式时，你可以分叉这个合约（制作副本）并将其作为一个起点
。当你这样做时，一切都非常清晰明了，你只需要更改该功能的内容，以重新定义哪些卡牌
是合法的规则集。

## 实战演习

1. 定义一个名为`isLegalCard`的函数。这个函数将接收一个参数，一个名
   为`card`的`CardInstance`，并返回一个`bool`。 该函数应具有`internal`和`view`属
   性。

2. 在函数体中，如果`card.mouldName`不在`bannedCards`映射中，它应该返回 true。 你
   可以用`return（！bannedCards [card.mouldName]）;`来做到这一点。
