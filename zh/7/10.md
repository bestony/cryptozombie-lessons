...
现在我们已经循环了这个玩家的所有卡牌，检查每张卡牌以确保它是合法的，如果是，则将
其添加到`newCards`数组中。
...
在我们`for`循环的上下文中，`uint` `i`代表玩家的编号（`0`是 1 号玩家、 `1`是 2 号
玩家）。所以对于第一个参数，我们可以使用`Player（i）` —— 我们的`enum`将
把`uint i`转换成`Player.Player1`或`Player.Player2`

## 实战演�

1. 在我们的第二个`for`循环之后（但仍在第一个循环中），如上所述调
   用`changes.changePlayerCardsInDeck`，并将`Player（i）`的卡牌更改为我们刚刚创
   建的`newCards`数组。
