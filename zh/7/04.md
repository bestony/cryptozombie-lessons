...
我们在第 3 章初始化的`GameState`现在可用于从当前游戏状态读取数据了。

在`beforeMatchStart（）`结束时，我们需要将数据作为`SerializedGameState`传递回游
戏服务器。

为此，我们需要声明一个名为`ZBSerializer.SerializedGameStateChanges`的自定义数据
类型，它将跟踪我们所做的所有更改。
