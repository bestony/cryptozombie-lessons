...
在本章中，我们将把讨厌的字节数组`serializedGameState`解构为我们可以轻松阅读和交
互的内容。

我们从`ZBGameMode.sol`访问的其中一个辅助结构叫做`GameState` —— 它是一个包
含`serializedGameState`中所有数据的数据结构。

让我们来看看里面是什么。

## `GameState`

`GameState`是一个嵌套`结构`，包含当前比赛的全部状态 —— 以及我们可能想要更改的所
有变量。 它看起来像这样：

> 注意：先不要担心理解不了每个部分都是干什么的 —— 尽管其中大多数都是不言自明，但
> 在接下来的章节中，我们将用真实世界的例子进行逐一介绍。这里只是为了给你一个整体
> 的概述。
...
简而言之：GameState 包含 2 个玩家，每个玩家都有一堆数据和一组卡牌。

稍后我们将看看如何与这个 GameState 结构进行交互。 但首先让我们
用`serializedGameState`中的数据对它初始化。
...
然后我们使用函数`init`来读取`serializedGameState`中的所有数据，并使用它来给我们
的`gameState`填充数据：
...
> 注意：`init`背后的逻辑是在`ZBSerializer.sol`中，并且有点复杂。 但这在幕后，你
> 不需要理解它，也能制作一个自定义的游戏模式 —— 除非你是个大书呆子 😉
>
> 重要的是‘僵尸战场’核心游戏逻辑使用与`ZBSerializer`完全相同的编码和解码格式，因
> 此游戏的两个部分能够通信并来回传递整个游戏状态。

现在数据填充在`gameState`中了，我们将能够使用`gameState`的子属性来访问游戏的当前
状态。

举个例子，如果你想看看 1 号玩家的牌组中第一张牌叫什么名字，你可以这样做：

```
gameState.playerStates[0].cardsInDeck[0].mouldName
```

或者，你也可以查看 2 号玩家的 Overlord 当前生命值，通过这样做：
