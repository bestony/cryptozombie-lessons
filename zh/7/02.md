...
“僵尸战场”自定义游戏模式合约的外壳就像 Solidity 中的任何其他智能合约一样开始，如
右侧所示。

首先要注意的是，我们从一个名为`ZBGameMode.sol`的文件导入，该文件
从`ZBSerializer.sol`和其他一些文件导入。

> 注意：其中一些文件非常大，因此我们没有将它们包含在本课程中。 你可以访问
> <a href="https://github.com/loomnetwork/zb_game_mode" target=_blank>此 github
> 代码库</a> 的完整源代码。 你不必担心阅读/理解不了这些文件的内容 —— 我们将解释
> 你需要知道的相关部分，以便创建自定义游戏模式。
...
make your life easier as a custom game mode developer. 由于我们的合
约`ExampleGame`继承自`ZBGameMode`，因此你可以访问我们创建的一系列常见数据结构和
辅助方法，从而让你轻松成为自定义游戏模式开发人员。

我们已经将它们抽象到一个库中，因此你不需要在每个自定义游戏模式中都使用相同的样板
逻辑。
...
基本上，核心游戏逻辑将在游戏的某些部分中（如果它们存在于你的代码中）寻找并调用合
约中的某些功能。

例如，`beforeMatchStart`将在比赛开始之前调用; `afterInitialDraw`会在玩家抽出第一
张牌后立即调用; `beforeTurnStart` _（已计划但尚未实现）_ 将在每个玩家回合开始时
触发。

这些 hook 允许你“点击”并修改那时的游戏状态，在游戏继续之前应用你的自定义规则。

我们将实现的第一个 hook 叫做`beforeMatchStart`，顾名思义, 它是应用在每场比赛开始
之前的 hook。
...
这些更改可能是对单个卡牌的平衡变化、在此游戏模式中允许/禁止卡牌的规则、以及对
overlord 生命值的更改或者 goo 小瓶的起始数量等。

> 从未玩过“僵尸战场”并且不确定这些术语指的是什么？ 请阅
> 读<a href="https://zombiebattleground.io/index.php/Gameplay" target=_blank>“僵
> 尸战场”游戏规则简介</a>。
...
这是一个有点讨厌的字节码块，其中包含**游戏的整个当前状态** —— 牌组中所有的玩家卡
牌、他们的 overlord 生命值、他们拥有的 goo 小瓶数量等等 —— 都浓缩成一个长长的字
符串。
...
that’s easy to work with — which we’ll look at in the next chapters. 在我们收到
这个有点讨厌的字节数组后，我们会将其转换为易于使用的格式 —— 将在后面的章节中介绍
。

之所以不以人类可读格式传递数据而以这种方式，是因为这是在“僵尸战场”核心游戏代码和
自定义游戏状态智能合约之间来回传递大量数据的更有效方法。

## 实战演习

1. 定义一个名为`beforeMatchStart`的`函数`。 它将采用一个参数，一个名
   为`serializedGameState`的`字节`。 这个函数应该是`external`。
