...
이 함수는 한 줄의 코드로 정의할 수 있지. 그럼 왜 따로 함수를 만들까? `if` 구문
안에 그냥 적지 않고 말이지.

이는 향후 새로운 게임 모드를 만들 때, 이 컨트랙트를 포크(복제본 만들기)하여 시작
점으로 쓸 수 있기 때문이지. 그리고 이렇게 할 때, 어떤 카드가 유효한지에 대한 규
칙을 재정의하려면 바로 저 하나의 함수 내용만 바꾸면 된다는 것이 매우 명확해질 것
이네.

## 직접 해보기

1. `isLegalCard`라는 이름의 함수를 하나 정의하게. 이 함수는 하나의 인수를
   `CardInstance`를 `card`라는 이름으로 받고, `bool`을 결과로 반환하네. 이 함수
   는 `internal`과 `view` 속성을 가져야 하네.

2. 함수의 내용에서, `card.mouldName`이 `bannedCards` 매핑에 없다면 참(true) 값을
   반환해야 하네. 이는 `return (!bannedCards[card.mouldName]);`와 같이 써서 할
   수 있네.
