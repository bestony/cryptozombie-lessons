...
우리의 중첩 `for` 반복문이 모두 동작한 후에, 두 플레이어들의 카드들은 어떤 금지
된 카드도 포함하지 않는 새로운 세트로 교체될 것이네(만약 플레이어가 처음부터 덱
에 금지 카드를 가지고 있지 않았다면, 아무런 변화가 없을 것이네).

우리는 또 각 플레이어의 시작 방어 값과 Goo 유리병의 개수도 바꿨네.

하지만 `changes`는 그저 메모리에 저장되는 로컬 데이터 타입이라는 점을 기억하게.
우리의 게임 모드를 완성하려면, 이 데이터를 핵심 게임 로직으로 전달해야 할 것이네
.
...
이름이 알려주듯이, 이는 `event`를 내보낼 것이네. 그리고 게임의 핵심 로직이 이 이
벤트를 수신하고 있지.

이 이벤트는 우리가 만들길 원하는 모든 변화에 대한 데이터를 포함하고 있네.
`serializedGameState`와 비슷한 형태로 말이네.
