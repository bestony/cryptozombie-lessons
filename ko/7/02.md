...
좀비 배틀그라운드 커스텀 게임 모드 컨트랙트의 뼈대는 솔리디티의 다른 여느 스마트
컨트랙트와 비슷하게 시작하네. 오른쪽에서 자네가 볼 수 있듯이 말이네.

처음 신경써야 할 부분은 `ZBGameMode.sol`이라는 이름의 파일을 임포트하는 것이지.
그 안에서는 `ZBSerializer.sol` 파일과 몇몇 다른 파일들을 임포트하지.

> 참고: 이러한 다른 파일들 중 몇몇은 그 크기가 꽤나 크네. 그러니 우리는 이 레슨
> 에서 그것들을 포함하지는 않을 것이네. 전체 소스 코드는
> <a href="https://github.com/loomnetwork/zb_game_mode" target=_blank>이 Github
> 저장소</a>에서 볼 수 있네. 하지만 이러한 파일들의 내용을 읽거나 이해하는 것에
> 대해서 걱정할 필요는 없네 - 자네의 커스텀 게임 모드를 만들기 위해 알 필요가 있
> 는 관련 부분들은 우리가 설명해나갈 것이네.

우리의 컨트랙트인 `ExampleGame`가 `ZBGameMode`를 상속하기 때문에, 여기서 자네가
더 편하게 커스텀 게임 모드를 개발할 수 있도록 하기 위해 우리가 만들어온 다양한
공통 자료 구조들과 헬퍼 메소드들을 사용할 수 있네.

우리가 이걸 라이브러리로 만들어 추상화해놨으니, 자네가 모든 커스텀 게임 모드에
매번 똑같은 템플릿 로직을 넣을 필요는 없을 것이네.

### "훅(Hook)" — 모든 커스텀 게임 모드의 핵심

모든 커스텀 게임 모드를 위한 첫 번째 단계는 바로 적어도 하나 이상의 **_훅_**을
구현하는 것이네.

기본적으로, 핵심 게임 로직에서는 게임의 특정 부분에서 자네의 컨트랙트에 존재하는
특정한 함수들을 찾아 호출할 것이네. 자네의 코드에 그 함수들이 존재한다면 말이지.

예를 들어, 매치가 시작되기 직전에는 `beforeMatchStart` 함수를 호출할 것이네. 플
레이어들이 그들의 첫 카드들을 뽑으면 그 즉시 `afterInitialDraw` 함수를 호출할 것
이고, 각 플레이어들의 턴이 시작할 때는 `beforeTurnStart` 함수(계획에 있으나 아직
구현 전)를 호출한다네.

이러한 훅들은 해당 시점에 자네가 "살짝 들어가서" 게임을 수정할 수 있도록 하네.
게임이 계속되기 전에 자네의 커스텀 규칙들을 적용하도록 하는 것이지.

우리가 구현할 첫 훅은 `beforeMatchStart`라고 불리네. 이름에서 알 수 있듯이, 이는
모든 매치의 시작 전에 적용되는 훅이지.
...
이 함수 내에서, 자네는 한 매치의 시작 시점에 적용하고 싶은 게임 로직의 모든 커스
텀 변경 부분들을 정의할 것이네.

이러한 변경들은 개별 카드의 밸런스 변경, 이 게임 모드에서 어떤 카드가 허용/금지
되는지에 관한 규칙들, 그리고 오버로드 체력이나 Goo 유리병의 시작 개수의 변경 등
이 될 수 있네.

> Zombie Battleground를 플레이해본 적이 없고 이 용어들이 뭘 의미하는지 잘 모르겠
> 는가?
> <a href="https://zombiebattleground.io/index.php/Gameplay" target=_blank>좀비
> 배틀그라운드의 게임 플레이 규칙들</a>에 관한 빠른 소개를 읽어보게.

## `bytes serializedGameState`

`beforeMatchStart`는 하나의 인수를 받네 — `serializedGameState`라는 이름의
`bytes` 값이지.

이는 **게임의 모든 현재 상태**에 대한 정보를 포함하는, 하나의 긴 문자열로 압축되
어 있는 알아보기 힘든 바이트 코드 덩어리이지 - 플레이어의 덱에 들어있는 모든 카
드들, 오버로드의 체력들, Goo 유리병들의 개수 등등을 가지고 있네.
...
이 알아보기 힘든 바이트 배열을 받은 후, 우리는 이를 다루기 쉬운 형태로 변환할 것
이네 - 다음 챕터들에서 알아볼 내용이지.

사람이 읽을 수 있는 형태의 데이터를 전달하지 않고 이러한 방식으로 처리하는 이유
는, 이 방식이 좀비 배틀그라운드 핵심 게임 코드와 커스텀 게임 상태를 위한 스마트
컨트랙트 간에 거대한 양의 데이터를 주고받기에 훨씬 효율적이기 때문이네.

## 직접 해보기

1. `function` 하나를 `beforeMatchStart`라는 이름으로 정의하게. 이 함수는 하나의
   매개 변수를 받을 것이네.`bytes`형이고 이름은 `serializedGameState`이어야 하지
   . 또 이 함수는 `external`이어야 하네.
