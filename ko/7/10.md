...
이제 우리는 이 플레이어의 카드들을 하나씩 접근하여 각 카드가 모두 유효한지 확인
했고, 유효하다면 `newCards` 배열에 그 카드를 추가하였네.

다음 단계는 `changes` 구조체에 우리가 이 플레이어의 원래 카드 배열을 새로운
`newCards` 배열로 교체하기를 원한다고 알려주는 것이지.
...
우리의 `for` 반복문의 내용에서, `uint` `i`는 플레이어의 번호를 나타내네(플레이어
1은 `0`, 플레이어2는 `1`). 그러니 첫 번째 인수로, 우리는 `Player(i)`를 쓸 수 있
네 - 그리고 우리의 `enum`이 `uint i`를 `Player.Player1`이나 `Player.Player2`로
변환할 것이네.

## 직접 해보기

1. 우리의 두 번째 `for` 반복문 바로 뒤에(그렇지만 여전히 첫 번째 반복문 안에서),
   위와 같이 `changes.changePlayerCardsInDeck`를 호출하고, `Player(i)`의 카드들
   을 우리가 만든 `newCards` 배열로 바꾸게.
