...
`changes`는 게임 상태의 특정한 부분을 변경하기 위해 우리가 사용할 수 있는 헬퍼
함수들의 집합을 가지고 있네.

예를 들어, 플레이어1의 시작 방어 값을 20에서 30으로 바꾸고 싶다면, 다음과 같이
할 것이네:
...
이에 익숙하지 않은 이들을 위해 설명하자면, 솔리디티에서 `enum`은 `uint`로 쉽게
상호 변환할 수 있는 유저 정의 데이터 타입을 만드는 방법이네.
...
왜 이런 것이 필요할까? 주로 코드의 가독성을 위해, 그리고 무엇이 실제로 일어나는
지 더 명료하게 하게 만들기 위해서이네.
...
첫 번째 예제에서, 우리가 플레이어1의 상태값을 변경하고 있다는 점을 쉽게 알 수 있
지. 반면에 두 번째 예제에서는, 코드를 읽는 사람이 `0`이 무엇을 의미하는지 알려면
함수 정의를 찾아봐야 할 것이네.

## 좀비 배틀그라운드 플레이어 상태값 바꾸기

위의 `changes.changePlayerDefense()`에 더해, 게임에서 다른 상태값들을 변형하기
위해 `changes` 객체에서 우리가 쓸 수 있는 다른 메소드들이 있네:

- `changePlayerCurrentGooVials` — 해당 라운드에서 플레이어가 현재 사용할 수 있는
  유리병의 개수이네. 매치가 시작할 때 기본값은 1이고, 매 턴마다 1씩 증가하지.
- `changePlayerCurrentGoo` — 해당 라운드에서 Goo가 채워진 유리병의 개수이네. 기
  본값은 매 턴의 시작 때 Goo 유리병의 개수와 같지.
- `changePlayerMaxGooVials` — 게임에서 플레이어가 가질 수 있는 최대 Goo 유리병의
  개수이네. 기본값은 10이지.
- `changePlayerInitialCardsInHandCount` — 매치 시작 시 플레이어가 받을 카드의 개
  수이네. 기본값은 3이지.
- `changePlayerMaxCardsInPlay` — 플레이어가 필드에 놓을 수 있는 좀비의 최대 개수
  이네. 기본값은 6이지.
- `changePlayerMaxCardsInHand` — 플레이어가 한 번에 손에 쥐고 있을 수 있는 최대
  카드의 개수이네. 기본값은 10이지.

이러한 함수들은 모두 `changePlayerDefense`와 같은 형태를 따르고 두 개의 인자를
받네: `Player` enum과 한 개의 `uint8`이지.

## 직접 해보기

다음 변수들을 바꿔보면서 우리 ExampleGame 모드의 커스텀 게임 규칙들을 만들어보도
록 하지.

1. Player1과 Player2의 시작 방어 값을 15로 설정하게.
2. 두 플레이어의 `CurrenGooVials`를 3으로 설정하고, 또 두 플레이어의
   `CurrentGoo`도 3으로 설정하게.
3. 두 플레이어의 `MaxGooVials`를 로 8로 설정하게.

위의 내용들은 모두 8줄의 코드로 만들어져야 하네. 메소드 이름들은 위에서 찾아볼
수 있고, 모든 메소드는 `changes.changePlayerDefense(Player.Player1, 30);`와 같은
형태를 따른다는 것을 생각하게.
