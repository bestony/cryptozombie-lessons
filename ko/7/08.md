...
이전 챕터에서 이어서, 우리는 지금 플레이어의 덱에 있는 각 카드에 하나하나 접근해
서 금지된 것인지 확인을 하려 하네.

## 플레이어의 덱에 있는 카드들을 하나씩 살펴보기

여기 두 플레이어의 덱에 있는 각 카드들에 하나씩 접근하고, 카드를 모두 "Slab"으로
바꾸는 방법의 예시가 있네 - Slab은 팬들이 가장 좋아하는 카드지 😉.
...
자네도 볼 수 있듯이, 중첩 `for` 반복문을 써서 먼저 각 플레이어의 목록에 하나씩
접근하고서, 그 플레이어의 각 카드의 목록에 하나씩 접근하네.

> 참고: 플레이어 덱에서 카드의 `mouldName`을 바꾸면 실제로 Slab이라는 이름의 카
> 드와 해당 카드를 완전히 바꾸게 되네. 그저 표시용 이름이 아닌 것이지.

## 직접 해보기

1. 위에서 본 것과 같이 두 번째 `for` 반복문을 만들어 각 플레이어의 모든
   `cardsInDeck`에 접근하도록 하게.

2. `if` 구문을 만들어 조건부에서 `isLegalCard` 함수를 호출하고 인수로
   `gameState.playerStates[i].cardsInDeck[j]`를 전달하게. 이 함수는 다음 챕터에
   서 정의할 것이네.

3. `if` 구문의 안에서, 우리의 새로운 배열에 카드를 복사해 넣도록
   `newCards[cardCount]`를 `gameState.playerStates[i].cardsInDeck[j]`로 설정하게
   .
