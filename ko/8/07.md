...
다음 단계에서는 각 플레이어의 카드를 하나씩 살펴보고, 동전 던지기처럼 난수를 생
성하여 어떤 플레이어가 카드를 얻을지 결정할걸세.

솔리디티에서 랜덤(pseudorandom) 동전 던지기를 여러 번 실행하는 방법은 다음과 같
다네:
...
난수에 대한 내용이 잘 생각나지 않는다면, 크립토 좀비 코스 1에서
<a href="https://cryptozombies.io/ko/lesson/4/chapter/4" target=_blank>레슨 4의
챕터 4</a>를 참고하게.

> 참고/주의: 난수를 생성하는 방법은 도박성 게임에서는 사용할 수 없다네. 블록 생
> 산자들이 시스템에서 자신들이 크게 이기지 않는 한 트랜잭션을 발행하지 않도록 할
> 수 있기 때문이지. 하지만, 우리가 하는 것과 같이 간단하게 덱을 섞는 정도로는 괜
> 찮다네.

## 직접 해보기

1. 먼저, 우리가 **Singleton**에 구현한 `for` 반복문 안의 내용을 전부 지우게. 여
   기에 우리만의 로직을 넣을 걸세.

2. 첫 번째 단계는 우리가 위에서 한 것처럼, 0 또는 1의 값을 가지는 `rand`라는 이
   름의 랜덤 `uint` 변수를 만드는 걸세. 하지만, 위의 예제의 `nonce` 대신, 우리는
   `player1CardCount + player2CardCount`를 사용할 수 있다네 — 이 두 값 중 하나가
   `for` 반복문이 실행될 때마다 하나씩 증가하리라는 것을 알기 때문이지.

3. 이제 난수를 생성했으니, 플레이어의 덱 중 하나에 카드를 추가할 수 있네.

a. `rand`의 값이 `0`인지 확인하는 `if` / `else` 구문을 만들게.

b. 참일 경우, 플레이어 1의 덱에 카드를 추가할 것이네. if 구문 안에
`player1Cards[player1CardCount]`에 `gameState.playerStates[i].cardsInDeck[j]`를
대입하는 코드를 작성하게.

c. 그리고, `if` 구문 안에서 `player1CardCount++`를 이용해 `player1CardCount`를
하나씩 증가시키게.

d. `else` 구문 안에서는 반대 작업을 하면 된다네: **플레이어 2**의 덱에 카드를 추
가하고 `player2CardCount` 값을 하나씩 증가시키게.
