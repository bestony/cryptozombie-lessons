...
중첩 `for` 반복문이 끝나면 모든 카드는 `player1Cards` 또는 `player2Cards`에 임의
로 추가되었을 걸세.

하지만 마지막 단계에서 우리는 이 카드들을 클라이언트에 다시 전달할 `changes` 데
이터에 실제로 불러오기 위해, 플레이어마다 `changes.changePlayerCardsInDeck` 함수
를 호출해야 한다네.

## 직접 해보기

1. *Singleton*에서 남겨진
   `changes.changePlayerCardsInDeck(Player(i), newCards, cardCount);` 줄을 잘라
   내게(또는 복사하고 지우게).

2. 두 번째 `for` 반복문의 **바깥쪽**에 위의 코드를 붙여넣게. 우리의 덱이 완전히
   구축되기 전에 이 반복문은 **양쪽** 플레이어들에 대한 실행을 마쳐야 한다네.

이 코드를 플레이어마다 한 번씩, 총 두 번 실행해야 한다네.

a. 처음에는 `Player(i)` 대신 첫 번째 인자로 `Player.Player1`를 전달해야 하네. 그
리고 두 번째와 세 번째 인자는 `newCards`와 `cardCount` 대신, `player1Cards`와
`player1CardCount`를 전달해야 한다네.

b. 이 코드를 다시 실행하기 위해 복사/붙여넣기를 하게. 하지만, 이번엔 Player 2를
위한 것이네 — 모든 참조를 Player1에서 Player2로 바꾸게.
