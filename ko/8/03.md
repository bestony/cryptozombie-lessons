...
_Munchkin_ 모드를 위한 마지막 단계는 `isLegalCard()` 함수 정의를 채우는 것이네
— 그러면 이 게임은 완전히 구현되는 것이지!

아주 간단하지 않은가?

*비용이 2 goo보다 많이 드는 카드는 모두 금지*한다는 *Munchkin*의 규칙을 떠올려보
게.

이 조건을 만족하는 카드를 필터링하는 가장 좋은 방법을 알아보기 위해,
`CardInstance`를 다시 살펴보게나:
...
위에서 확인할 수 있듯이, `CardInstance`에는 `gooCost` 속성이 있다네. 그래서 어떤
카드가 _Munchkin_ 모드에서 유효한지 테스트하려면, 단순히 `card.gooCost`의 값이 2
보다 작거나 같은지 확인해보면 되는 것이지.

## 직접 해보기

`isLegalCard()` 함수가 `bool` 타입을 반환하므로, 이 함수 정의는 한 줄의 코드로
채울 수 있다네 — `return` 구문으로 말이지.

이 함수는 `card.gooCost`의 값이 2보다 작거나 같다면 `true`를, 아니면 `false`를
반환해야 한다네.
