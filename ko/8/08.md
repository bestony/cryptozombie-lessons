...
우리는 각 카드를 위한 랜덤 동전 던지기를 구현했고, 이 코드를 통해 동전 던지기의
결과에 따라 해당 카드를 플레이어의 덱에 더할 것이네.

하지만 여기에 문제가 있지 - 동전 던지기로 플레이어1이 절반보다 많은 카드를 받게
되면 어떻게 될까?

이렇게 되면 게임이 공정하지 않을 것이네 - 하지만 이 경우 그의 카드를 저장하기 위
해 우리가 만든 배열에 오버플로우가 발생할 것이네 🙃.

그러니 이런 일이 발생하지 않도록 우리 코드에 확인하는 부분을 추가해야 하네.

## 직접 해보기

우리의 `if` 구문 전에, **또 하나의** `if` 구문을 추가하여 배열에 오버플로우가 발
생할 경우를 검사하도록 하지.

1. `if` 구문을 하나 추가하여 그 조건에서 `player1CardCount + 1`가
   `gameState.playerStates[0].cardsInDeck.length`보다 큰지 검사하도록 하게.

2. 만약 조건이 참이라면, `rand = 1`로 설정하게(이렇게 하면 우리의 다음 if/else
   구문에서 플레이어1 대신 플레이어2에게 카드를 줄 것이네).

3. `else if` 절을 추가하여 `player2CardCount + 1`가
   `gameState.playerStates[1].cardsInDeck.length`보다 큰지 검사하도록 하게.

4. 만약 조건이 참이라면, `rand = 0`으로 설정하게(즉 카드를 플레이어1에게 주는 것
   이네).
